...
The side-port CAD proposal is in cad/usb-port-study.md (USB-MOUNT-01 v0.3). The passive port PCB is now routed in hardware/usb-port: 26×7.6×1 mm, USB4105-GF-A, two independent 5.1k Rd, six harness terminals. KiCad 10.0.6 DRC passed, but no separate schematic/ERC or physical assembly exists. Pad/hole geometry drives the updated CAD, connector face X=59.1 mm. Use the KiCad Python in build/tooling/kicad-10.0.6 to regenerate; setup/evidence is in hardware/usb-port/README.md. Keep the charger separate; never add another Rd pair on the main board, preserve quick battery extraction and adjustable electronics.
