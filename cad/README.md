...
14 сентября: добавлена [компоновка бокового зарядного USB-C](usb-port-study.md) с [переключением закрытого порта и кабеля в 3D](usb-port-study-viewer.html). Основание и макет платы экспортированы; геометрия проверена, PCB и фиксация накладки ещё не разработаны.
