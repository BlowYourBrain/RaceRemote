...
**Полная CAD-сборка RaceRemote пока не создана.** Здесь находятся 3D-модели батарейного макета и держателя в исходной раме zcar. Крепления выбранных мотора/серво, плат, камеры, антенны, выключателя и USB, проводка, кузов и магниты ещё не сведены в общую модель. Изображения ниже — сечения для проверки батарейного узла, а не чертежи готовой машинки. Существующие редактируемые 3D-файлы — `battery-layout.scad` и `battery-holder.scad`; STL предназначены для отдельных деталей.
