...
Общая ESP32 для видео и управления — **архитектурное предложение для серии**. Теперь подготовлены [совместные сборки XIAO](../../RC_CAR_ESP32/docs/combined-bench.md), но на плате они ещё не проверены. До принятия нужны совместный FPV ≥30 FPS, задержка команд, отключение/зависание камеры с сохранением управления и независимая остановка по watchdog при потере команд. Удаление второго контроллера из рисунка и успешная сборка не доказывают FAIL-01/CAMERA-03. Работа без установленной камеры должна сохраняться.
