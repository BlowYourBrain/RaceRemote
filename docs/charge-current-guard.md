...
14 сентября 2026. CHARGE-OC-01 v0.2 — **схема-кандидат**, без платы и измерений.
[Схема INA300 с удержанием и шунтом](../hardware/charge-overcurrent/README.md).
Проверено [покрытие токов ячеек CHARGE-SENSE-01](charge-sense-paths.md):
один шунт в плюсе пропускает часть тока нижней ячейки при верхней
балансировке. В DC-контрпримере 400 мА в нижней соответствуют 302 мА
на шунте; 348 мА нельзя объявлять порогом для обеих ячеек. Два канала
исследуются как следующий вариант, но ещё не добавлены в схему или смету.
Цель — остановить нештатный заряд BQ25887 после
...
Стрелки описывают функцию, не монтаж. Сигнальная цепь CD и интерфейс INA300
нарисованы на отдельных листах; окончательное место шунта ещё не выбрано.
Шунт должен видеть зарядную ветвь; тягу через него не
...
диапазон готового узла. В v0.2 отдельно добавлен TCR выбранного шунта
RL2512FK-070R1L ±600 ppm/°C: при предполагаемых 125 °C тела частичная оценка
расширяется до 313…388 мА. TCR R701, разводка и пульсации ещё не учтены.
Температуры в расчёте — исследуемые точки, не назначенный режим машинки.
Допустимый зарядный ток и импульсный режим LW неизвестны;
...
и 484 мВт. Выбран кандидат RL2512FK-070R1L с номиналом 1 Вт при 70 °C;
тепловой режим на нашей плате и импульсная стойкость ещё не квалифицированы.
...
| 4 | ENABLE | В v0.2 напрямую к AUX; MCU не имеет линии отключения |
| 5 | ALERT | На FAULT_BUS_N; только существующая подтяжка R401 10 кОм, без второй на INA300 |
| 6 | LATCH | В v0.2 напрямую к AUX, без программного сброса/прозрачного режима |
...
| EP | Теплоплощадка | На схеме земля, локальный номер 11; окончательная разводка открыта |

Даташит описывает очистку через LATCH LOW не менее 20 мкс при токе ниже
порога. В v0.2 такой управляющей линии нет: LATCH соединён с AUX, замыкать
его на землю нельзя. Предлагается восстановление только после отключения
USB и новой процедуры запуска. Очистка от снятия питания, полный разряд AUX,
отсутствие подпитки и запрет на переходе пока не доказаны. Потеря питания
детектора не равна разрешению заряда.
...
Следующий шаг — согласовать два измерительных пути с защитой батареи и
землями либо доказать достаточный запас на обходной ток для одного канала;
затем завершить схему и восстановление после ошибки для испытания на имитаторе.
...
Продолжение: [CHARGE-PERMIT-01 v0.4](../hardware/charge-permit/README.md)
...
INA300 теперь имеет отдельный лист с постоянно включённым удержанием;
его J703 сопоставлен с permit J402. Повторные ARM при удержанной токовой
ошибке в цифровой модели не разрешают заряд. Переходы питания и ток не проверены.
