...
Для A теперь создан [PACK-BAL-01 v0.1](../hardware/pack-balancer/README.md),
отдельный лист BQ29209 на стороне ячеек: ERC/35 выводов/пять ошибок проверены.
Это делает резервный вариант конкретнее, но не выбирает его победителем.
Сильная сторона A — отсутствие программного сброса зарядного тока; пробел —
нормальное регулирование напряжения каждой ячейки, которое не заменяется
вторичным OVP BQ29209. Также нужны сопряжение батарейной области с USB-логикой
и первичная защита. Полные стоимость/размер по-прежнему не сравнены.
