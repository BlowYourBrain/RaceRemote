...
14 сентября [измерена область установки серво](../cad/servo-clearance/README.md): номинальный проход 22,8 мм, корпус исходной справочной модели 22,4 мм, брусок выбранного SG90 23 мм. Нужны измерения одного образца и вероятная локальная адаптация боковых упоров; передняя нижняя коллизия бруска может зависеть от реальной формы выступа вала. Полная замена серво или срезание всех зон пересечения пока не обоснованы.

Текущие продолжения: [регулируемые крепления 0.3](../cad/adjustable-layout.md) и [примерка конкретных компонентов 0.4](../cad/candidate-fit.md). Габарит из карточки SG90 даёт пересечение с рамой; это требует точного чертежа/измерения, а не утверждения совместимости по названию. Для XIAO использована официальная STEP-геометрия 2023, её соответствие текущей OV3660 открыто.

После уточнения пользователя кузов стал явным ограничением компоновки (BODY-03). [Вариант CAD 0.2](../cad/body-study.md) показывает предложенную оболочку и снижение высоты: это не геометрия конкретного Mini-Z. В исходной системе zcar перед находится по +Y, ведущие колёса сзади по меньшему Y; прежняя подпись просмотрщика исправлена. Общая плата ESP32/камеры и низкий преобразователь предложены ради места под крышей, но ещё не выбраны/испытаны.
