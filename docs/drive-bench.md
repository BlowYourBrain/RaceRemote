...
Обновление 14 сентября 2026: реализован [план выходов привода 0.1](drive-output-plan.md) — масштабирование газа, нарастание, пауза реверса и STOP из независимой задачи. Пока это вычисления и диагностика `drivePlan`, без GPIO/LEDC и калибровки серво. Ниже остаётся порядок физической проверки; он не отмечен выполненным.
...
Для выбранного направления XIAO S3 Sense подготовлена отдельная [карта GPIO/таймеров и схема стенда](xiao-drive-wiring.md). Номера WROOM ниже на XIAO не переносить.
...
Уточнение таблицы: при VCC=6 В для ViH приведены 2,2 / 3,5 / 6 В (min/typ/max); 3,5 В — не минимальное требование к входу. Прямое управление 3,3 В обосновано для начальной проверки, хотя весь диапазон питания/температуры этим не подтверждён. Подготовлена [схема проверки входов и выходов без мотора](ta6586-input-check.md): 100 Ом последовательно, 10 кОм на землю и нагрузочные делители, различающие выключение и тормоз. Не подтягивать GPIO ESP32 к напряжению аккумулятора.
...
Сейчас v1 сбрасывает логические значения и закрывает сессию, а новый программный план назначает нулю FI=BI=0 как предложение выбега. На ножки TA6586 это ещё не подаётся. Предел 25%, нарастание и пауза реверса 200 мс — программные предложения, не проверенные моторные режимы. Для физических выходов остаётся определить:
...
Не делать мгновенный полный реверс способом торможения. Частота PWM, окончательный предел газа и пауза реверса пока не утверждены. Нулевая команда не доказывает, что колёса уже остановились: для STOP-01 дополнительно измерить время/путь остановки на полу при выбранном ограничении скорости.
