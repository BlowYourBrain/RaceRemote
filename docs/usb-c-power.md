...
Для текущего кандидата XIAO Sense отдельно проверена заводская схема и подготовлен [вариант со штатным USB-C](xiao-usb-power.md): боковой 5V связан напрямую с USB VBUS; для батарейного питания предлагается отдельный понижающий DC-DC через диод к BAT0. Это сохраняет настоящую батарею 2S и отдельный зарядник 2S. Схема требует второго DC-DC, проверки штатного зарядника 1S без батареи на его выходе и нового места в CAD; не является готовой инструкцией подключения. Общая схема ниже остаётся функциональной альтернативой с отдельным выбором источника.
