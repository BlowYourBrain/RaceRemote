...
## Дополнительная карточка из открытого Chrome, 14 сентября

В обычном Chrome была открыта [EGBO 1005007553147829, SKU 12000041268860668](https://aliexpress.ru/item/1005007553147829.html?sku_id=12000041268860668).
Карточка прочитана через Windows UI Automation, без навигации, изменения
варианта или добавления в корзину. Заголовок: «Мини модуль зарядки литиевых
батарей EGBO 1A/1.2A»; выбранный вариант — **5PCS type-c 1A**; магазин EGBO Store.
Показаны 111 ₽ (121 ₽ зачёркнуто), доставка в Чебоксары 40 ₽, ПВЗ 24–30 сентября.
Одновременно интерфейс пишет «Цена за 1 штуку»/«Количество 1»: единицу продажи
относительно обозначения 5PCS отдельно не проверяли, цену одной платы не вычисляем.

В характеристиках поле «Напряжение электропитания» содержит **4.2V**.
Это не доказательство режима 2S/8,4 В: точная микросхема, схема и параметры
заряда этой комплектации не прочитаны. Названия TP4056 встретились в соседних
рекомендациях, их нельзя приписывать выбранному товару. Плата не включена
в закупку для LW; пока это карточка без подтверждения соответствия 2S.
Ток 1 А в названии также не устанавливает допустимый режим наших батарей.
Ничего не заказано. Ниже сохранено основное исследование зарядного узла.

## Основное исследование

Добавлена [схема USB-входа](../hardware/usb-input-gate/README.md), USB-INPUT-GATE-01 v0.2: TPS25200 и независимое питание логики TPS70933, AUX — выход 3,3 В. ERC/12 выводов/шесть намеренных ошибок проверены; распознавание источника и обработка просадки ещё открыты. Условный clamp до 5,55 В не подтверждает прежний диапазон CHG_VBUS до 5,5 В и не ограничивает все выбросы; прямое объединение схем пока не принято.
...
**15 сентября — уточнение общих проводов:** [POWER-GROUND-01](power-ground-domains.md) показывает обход идеальной защиты в минусе через два USB к общему ПК/хабу, если зарядная земля сидит на RAW_N, а XIAO — на PROTECTED_N. Плюсовой выключатель это соединение не разрывает. Обычные внешние земли предложены со стороны PROTECTED_N, но совместимость зарядника со средней точкой/балансировкой ещё не доказана; это не готовая схема BMS. Не присоединять напрямую землю батарейного монитора к DRIVE_GND.
