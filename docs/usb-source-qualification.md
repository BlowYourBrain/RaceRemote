...
14 сентября 2026. USB-SOURCE-01 v0.3 — **предложение интерфейса**, не готовая
...
## PI3USB9201: кандидат с раздельными классами

В v0.3 добавлен [USB-BC-01 v0.1](../hardware/usb-detector/README.md):
схема PI3USB9201 с питанием AUX, отключением по умолчанию и отдельными
статусами DCP/CDP/SDP/legacy. 12 выводов, 9 позиций BOM, ERC=0 и три
намеренные ошибки проверены. Схема контроллера и фактическая классификация
ещё отсутствуют. В отличие от исследованной пары выше, этот вариант
требует автономного MCU с I²C. Зарядка не должна зависеть от XIAO/телефона.

Результат 0x02 очищается при чтении: INTB/старый кэш не являются постоянным
разрешением. Нужны актуальное подключение CC/VBUS, сброс при ошибках,
аппаратная обработка зависания MCU и согласованная политика Type-C/BC.
Схема не включает TUSB и не меняет Rd нынешнего порта. Прежде чем выбрать
комплект, сравнить полные зарядные A/B по стоимости/габаритам с учётом MCU;
BQ25886 больше нельзя считать гарантией отсутствия MCU во всём изделии.
