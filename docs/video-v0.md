...
**Смена машинки, 15 сентября:** [CAR-VIDEO-01](car-video-selection.md) закрывает прежний поток при изменении адреса/порта управления и подставляет URL совместного XIAO. Ручной адрес отдельной камеры и явное «Смотреть» сохраняются; идентичность камеры не проверяется.
...
На экране управления выбрать адрес машинки, включить флажок «Видео», проверить подставленный `http://<адрес машинки>:81/stream` и нажать «Смотреть». Для отдельной камеры URL можно изменить вручную. IP/порт конкретной прошивки нужно сверить. Смена машинки закрывает старый поток; камера и управление запускаются отдельно. Кнопка «Выключить» останавливает только видео; «СТОП» по-прежнему останавливает управление. Ошибка видео не выключает исправное управление. Повторное подключение видео выполняется вручную и не требует нового ARM для уже работающего управления.
...
- URL подставляется по адресу управления для совместного XIAO и допускает ручное изменение. Обнаружение/проверка личности камеры, авторизация и согласование режима потока ещё не реализованы. Редиректы и автоматические повторы HTTP отключены.
