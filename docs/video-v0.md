...
После паузы/ухода приложения в фон обе сессии закрываются, старое изображение очищается, автоматического возобновления нет. Если видео не нужно, убрать флажок: остаются обычные органы управления. Видеопанель пока стендовая; компоновка с крупным FPV-изображением и окончательный способ управления пальцами ещё требуют проработки.
...
«Получено» — число полных JPEG за окно примерно 1 с. «Отрисовано» — число разных полученных кадров, переданных в `Canvas` через `onDraw`, за то же окно. Дополнительно в logcat с тегом `RaceRemoteVideo` записывается частота декодирования. Частота повторного рисования одного Bitmap не засчитывается.

Это **не оптическое измерение реально показанных экраном кадров** и не задержка от сцены до телефона. Без метки захвата/последовательности от камеры получатель также не может распознать повторную передачу старой фотографии как новый JPEG. Серверный `X-Timestamp` пока не используется; часы камеры и телефона не вычитаются. Для CAMERA-03 всё ещё нужны сцена с изменениями, внешний замер, согласованный режим и работа управления одновременно с реальной камерой.
