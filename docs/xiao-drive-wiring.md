...
15 сентября подготовлена [альтернатива от общего Waveshare 5 В](../hardware/xiao-logic-power/README.md): LDO около 4 В и отдельный ключ, управляемый штатным сервисным VBUS. Есть схема и DC-расчёт, но заметны потери/малый запас при просадке; переходы и теплоотвод не проверены. Это не утверждённая замена второго DC-DC и не инструкция включать привод.
