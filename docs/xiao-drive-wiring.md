...
XIAO на этом этапе питается от USB; выход DC-DC питает только серво. Батарею 2S не подключать к BAT XIAO: [Seeed указывает для BAT 3,7 В](https://wiki.seeedstudio.com/xiao_esp32s3_getting_started/). Автономное питание XIAO от 5 В и общий порт зарядки требуют отдельной развязки от USB, остаются в [USB-C power](usb-c-power.md). Мотор питается от моста, допустимость полного диапазона 2S для конкретной обмотки проверяется отдельно.
...
Подтяжки FI/BI дают 00 при отключённых GPIO — отсутствие тяги и выбег, а не гарантированный короткий тормозной путь. Для работающей прошивки ещё предстоит реализовать применение STOP из независимой задачи, выбрать выбег/электротормоз по току и пути остановки, ограничение газа и переход через нейтраль при реверсе. Нельзя добавлять запись выходов только в сетевой цикл: при его задержке последний PWM продолжит работать. Отказ камеры не должен менять разрешение управления.
