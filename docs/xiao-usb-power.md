...
15 сентября: [подготовлена отдельная альтернатива питания логики 0.2](../hardware/xiao-logic-power/README.md) от общего Waveshare через TPS73701/MAX40200. Внешний провод обнаружения USB удалён; раздельные зарядный/сервисный порты сохраняются. Схема и расчёт ещё не подтверждают пригодность к монтажу. Прежний дополнительный buck ниже остаётся резервным кандидатом.

13 сентября 2026. Статус: **предшествующее исследование общего порта, не выполненный монтаж**. После разрешения отдельного зарядного порта основное направление заменено [раздельными зарядным и сервисным USB-C](usb-c-power.md#отдельный-зарядный-порт--текущее-предложение). GIFT-01 больше не требует прошивки через зарядный разъём. Электрические выводы о XIAO ниже сохраняют силу для сервисного питания; зарядник и защита батареи не готовы.

Ранее агент предложил штатный USB-C XIAO как основной вариант следующего стенда с отдельным зарядником 2S от VBUS. Это предложение заменено, а не подтверждено монтажом. Схема и таблица ниже описывают именно прежний общий порт: в новом варианте питание зарядника берётся от отдельного зарядного USB-C, а не VBUS XIAO. Прямое подключение батареи 2S к штатному заряднику XIAO не предлагается.
