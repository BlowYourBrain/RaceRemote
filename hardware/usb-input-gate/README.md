...
## Продолжение: отдельное питание CD

[CHARGE-BIAS-01](../../docs/charge-bias.md) добавляет U406 TPS70933 после
защищённого входа для supervisor/TMUX. Он допускает условные 5,55 В на
своём входе и выдаёт отдельные 3,3 В. Это устраняет прямую подачу слишком
высокого напряжения на TMUX, но не расширяет контракт ядра и не закрывает
переходные процессы или малый запас напряжения. Схема данного входного
ключа остаётся v0.2; автоматически объединять сети пока нельзя.
