# Пассивная плата зарядного USB-C, ревизия 0.2

14 сентября 2026. **Схема и разводка выполнены, ERC/DRC и сравнение схемы с PCB прошли, плата не изготовлена.** Это плата разъёма с двумя резисторами, подключаемая к отдельному зарядному узлу. Зарядного контроллера, защиты аккумулятора, ограничения входного тока и согласования PD/QC здесь нет. Батарею 2S к VBUS не подключать.
...
[Схема KiCad](usb-port.kicad_sch) · [Схема PDF](schematic.pdf) · [ERC](../../docs/evidence/usb-port-erc.json) · [Проверка соединений и намеренных ошибок](../../docs/evidence/usb-port-pcb-verification.json)

![Электрическая схема платы порта](../../docs/evidence/usb-port-schematic.png)
...
В отчётах сохранены стандартные отключённые проверки KiCad; у площадок жгута и крепёжных отверстий нет отдельных courtyard. Их механические зоны проверяются явно в PCB/CAD. Исключения для получения нуля не добавлялись. **ERC: 0 замечаний; сравнение схемы с PCB: 0 расхождений.** Дополнительно сверены 27 логических выводов экспортированного netlist с площадками PCB, включая повторные контакты питания и экрана. В отдельных копиях намеренно перепутан H3 с CC2 и удалён R1: сравнение обнаруживает соответственно `net_conflict` и `missing_footprint`.

Схема задаёт соединения самостоятельно, без чтения цепей PCB. Символ USB взят из KiCad 10.0.6, обозначение экрана адаптировано `SH` → `S1` под GCT; происхождение и изменения — в [NOTICE](SYMBOLS-NOTICE.md). Для A8/B8 созданы явные NC-цепи KiCad, проводников на них нет. R1: контакт 1 → GND, 2 → CC1; R2: 1 → CC2, 2 → GND. UUID связывают символы с посадочными местами. Изменение 0.2 затрагивает электрические метаданные PCB; расположение и размеры площадок/отверстий сохранены, 42 записи CAD проверены по текущей плате.

ERC/DRC не доказывают допустимый ток/нагрев, отсутствие дефектов пайки, стойкость к статике и работоспособность зарядки. Зарядный узел 2S, крепление жгута и физические измерения остаются открытыми.
...
& build/tooling/kicad-10.0.6/bin/python.exe tools/build_usb_port_schematic.py
& build/tooling/kicad-10.0.6/bin/python.exe tools/verify_usb_port.py
& build/tooling/kicad-10.0.6/bin/kicad-cli.exe sch export pdf hardware/usb-port/usb-port.kicad_sch -o hardware/usb-port/schematic.pdf
uv run --with-requirements tools/cad-requirements.txt python tools/build_usb_port_study.py
```

`verify_usb_port.py` запускает штатные ERC/DRC с `--schematic-parity`, проверяет все разделы JSON независимо от кода возврата CLI, сопоставляет netlist/PCB и проверяет две намеренные ошибки в `build/usb-port-pcb/negative-controls/`. На физические устройства скрипт не воздействует. Для просмотра схемы достаточно проектных библиотек; повторно извлекать полные библиотеки KiCad не нужно. Генератор формата схемы основан на [официальном описании KiCad](https://dev-docs.kicad.org/en/file-formats/sexpr-schematic/).
