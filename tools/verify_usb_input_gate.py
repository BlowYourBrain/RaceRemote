...
def main():
    ...
    for name,old,new,position in [
        ...
        ('output_terminal_bypasses_gate','GATED_VBUS','USB_RAW_VBUS','170.18 223.52 180'),
        ...
        changed=original.replace(token,token.replace('"'+old+'"','"'+new+'"',1))
    ...
    report={'date':'2026-09-14','contract':'USB-INPUT-GATE-01 v0.1',
            'kicad_version':erc_data['kicad_version'],'erc_violations':0,'ic_pins_including_EP_checked':7,
            ...
            'not_verified':['PCB/thermal footprint and manufacture','Source classification and pre-gate auxiliary supply',
